...
with tab1:
    ...
    for barre, valeur, pct, couleur in zip(barres, valeurs, pourcentages, couleurs):
        ax.text(barre.get_x() + barre.get_width()/2, barre.get_height() + 10, f"{valeur}\n({pct}%)", ha="center", va="bottom", fontsize=11, fontweight="bold", bbox=dict(boxstyle="round,pad=0.3", facecolor="white", edgecolor=couleur))
...
with tab2:
    ...
    for ax, (_, row) in zip(axes, credit.iterrows()):
        ...
        ax.text(0, 0, f"{taux}%", ha="center", va="center", fontsize=13, fontweight="bold")
...
with tab3:
    ...
    for barre, valeur in zip(barres, zone.values):
        ax.text(valeur + 1, barre.get_y() + barre.get_height()/2, f"{valeur}%", va="center", fontsize=14, fontweight="bold")
    ax.axvline(taux_refus_global, color="black", linestyle="--", linewidth=1.2)
    ax.text(taux_refus_global, len(zone) - 0.15, f" Moyenne : {taux_refus_global}%", fontsize=10, style="italic", va="bottom")
    ax.set_title(f"{zone.idxmax()} : la zone la plus exposée au refus de prêt", fontsize=15, fontweight="bold", pad=25)
    ax.set_xlabel("Taux de refus (%)", fontsize=11)
    ...
    ax.legend(handles=patches, loc="center left", bbox_to_anchor=(1.02, 0.5), frameon=False, fontsize=10)
    plt.tight_layout()
    st.pyplot(fig)
